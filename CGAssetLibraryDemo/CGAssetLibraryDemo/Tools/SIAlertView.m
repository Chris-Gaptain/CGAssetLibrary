...
#define SIColorRGB(R,G,B,A) [UIColor colorWithRed:R/255.0 green:G/255.0 blue:B/255.0 alpha:A]
...
@property (nonatomic, assign) BOOL isToast;         //判定是不是toast显示
...
- (void)drawRect:(CGRect)rect
{
    ...
    switch (self.style) {
        ...
        case SIAlertViewBackgroundStyleNormal:
        {
            [[[UIColor whiteColor]colorWithAlphaComponent:0.1] set];
            CGContextFillRect(context, self.bounds);
        }
            break;
    }
...
+ (void)initialize
{
    ...
    appearance.titleColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.000];
    appearance.messageColor = [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.000];
    appearance.titleFont = [UIFont boldSystemFontOfSize:16];
    appearance.messageFont = [UIFont systemFontOfSize:14];
    appearance.buttonFont = [UIFont systemFontOfSize:15];
...
- (void)showToastIsAnimated:(BOOL)animated
{
    self.viewBackgroundColor = SIColorRGB(80, 80, 80, 1.0);
    self.messageFont = [UIFont boldSystemFontOfSize:16];
    self.messageColor = [UIColor whiteColor];
    self.backgroundStyle = SIAlertViewBackgroundStyleNormal;
    self.isToast = YES;
...
- (void)validateLayout
{
    ...
    CGSize messageSize = [self heightForMessageLabel];
    CGFloat messageHeight = messageSize.height;
    self.containerView.transform = CGAffineTransformIdentity;
    if (self.isToast) {
        CGFloat containWidth = CONTENT_PADDING_LEFT * 2 + messageSize.width;
        self.containerView.frame = CGRectMake((kMainScreenWidth - containWidth) * 0.5, top, containWidth, messageHeight);
    }else{
...
//        height = = [self heightForTitleLabel];
        self.titleLabel.frame = CGRectMake(CONTENT_PADDING_LEFT, y, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, titleHeight);
        self.separateImageView.frame = CGRectMake(0, y + titleHeight , self.containerView.bounds.size.width, 0.5);
        y += titleHeight;
    }else{
        y += GAP;
    }
    ...
    if (self.messageLabel) {
        ...
        if (self.isToast) {
            ...
            CGFloat messageY = (self.containerView.height - messageHeight) * 0.5;
            
            
            self.messageLabel.frame = CGRectMake(CONTENT_PADDING_LEFT, messageY, messageSize.width, messageHeight);
            self.messageLabel.textAlignment = NSTextAlignmentCenter;
        }else{
            self.messageLabel.frame = CGRectMake(CONTENT_PADDING_LEFT, y, self.containerView.bounds.size.width - CONTENT_PADDING_LEFT * 2, messageHeight);
            SIAlertItem *item =[self.items objectAtIndex:0];
            if (item.type == SIAlertViewButtonTypeContenLeft) {
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2) {
            CGFloat width = self.containerView.bounds.size.width * 0.5;
            UIButton *button = self.buttons[0];
            button.frame = CGRectMake(0, y, width, BUTTON_HEIGHT);
            button = self.buttons[1];
            button.frame = CGRectMake(width, y, width, BUTTON_HEIGHT);
            
            UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:button.bounds byRoundingCorners:UIRectCornerBottomLeft|UIRectCornerBottomRight cornerRadii:CGSizeMake(self.cornerRadius, self.cornerRadius)];
...
//		height += [self heightForTitleLabel];
    ...
    if (self.message) {
        ...
        height += [self heightForMessageLabel].height;
    }
    if (self.items.count > 0) {
        if (height > CONTENT_PADDING_TOP) {
            height += GAP;
...
- (CGSize)heightForMessageLabel
{
    CGFloat minHeight = MESSAGE_MIN_LINE_COUNT * self.messageLabel.font.lineHeight;
    CGSize size = CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2, minHeight);
    if (self.messageLabel) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] <= 7.0f) {
            ...
            size = [self.message labelAutoCalculateRectWithFont:self.messageLabel.font MaxSize:CGSizeMake(CONTAINER_WIDTH - CONTENT_PADDING_LEFT * 2, maxHeight)];
        }
       
        size.height = MAX(minHeight,ceil(size.height));
...
- (void)updateTitleLabel
{
	if (self.title && ![@"" isEqualToString:self.title]) {
		if (!self.titleLabel) {
            ...
            self.separateImageView.backgroundColor = SIColorRGB(224, 224, 224, 1);
#ifndef __IPHONE_6_0
            ...
            [self.containerView addSubview:self.separateImageView];
#if DEBUG_LAYOUT
        ...
        [self.separateImageView removeFromSuperview];
        self.separateImageView = nil;
	}
    [self invaliadateLayout];
}
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
    ...
    button.layer.borderColor = SIColorRGB(224, 224, 224, 1.0).CGColor;
	[button setTitle:item.title forState:UIControlStateNormal];
	UIImage *normalImage = nil;
	UIImage *highlightedImage = nil;
    
    UIColor *titleColor = SIColorRGB(31,182,196,1.0);
	switch (item.type) {
		case SIAlertViewButtonTypeCancel:
            ...
            titleColor = SIColorRGB(153, 153, 153, 1.0);
			break;
		...
		default:
			...
			[button setTitleColor:SIColorRGB(31,182,196,1.0) forState:UIControlStateNormal];
            [button setTitleColor:SIColorRGB(31,182,196,1.0) forState:UIControlStateHighlighted];
			break;
	}
